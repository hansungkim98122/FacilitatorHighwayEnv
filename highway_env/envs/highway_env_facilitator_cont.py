...
class HighwayEnvFacilitatorCont(AbstractEnv):
    """
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            "vehicles_count": 10, #number of npc vehicles
            ...
            "SEPARATION_COST": 4,
            "LANE_CHANGE_COST": 5,   # The reward received at each lane change action.
    ...
    def _agent_reward(self, action: int, vehicle: Vehicle) -> float:
       ...
       """
        ...
        reward = self.config["COLLISION_COST"] * (-1 * float(vehicle.crashed)) \
                 ...
                 + self.config["SEPARATION_COST"] * (np.log(self.cav_d_thresh/min_d) if self.cav_d_thresh/min_d < 1 else 0) \
                ...
                #  + self.config["INFEASIBLE_COST"] * (-1 * float(not vehicle.feasible)) * float(not vehicle.crashed) \
    ...
    def _is_terminated(self) -> bool:
        """The episode is over if the ego vehicle(s) crashed."""
...
class HighwayEnvFast(HighwayEnvFacilitatorCont):
    """
